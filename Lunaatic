...
local function buildCatchEmbed(catchData, rarityLabel, embedColor, options)
    ...
    return {
        embeds = {{
            title       = "🔒 LUNAATIC PRIVATE - SERVER HOOK",
            ...
            footer = {
                text = string.format("LICENSE TO LUNAATIC • %s", catchData.time)
            }
        }}
    }
end
...
testBtn.MouseButton1Click:Connect(function()
    ...
    local testEmbed = {
        embeds = {{
            title       = "🔒 LUNAATIC PRIVATE - SERVER TEST CONNECTED",
            ...
            footer = {
                text = string.format("LICENSE TO LUNAATIC • %s", os.date("%d/%m/%Y %H:%M"))
            }
        }}
    }
